...
/**
 * A linear gradient fill style. The liner gradient is filled from a start to an
 * end position. Along this line, so-called "colour stops" can be set, which
 * influence how the colour is changed along the gradient. For example: Setting
 * a colour stop with black at start position and a colour stop with white at
 * the end position will lead to a gradient which goes through all shades of
 * grey. They can be more than two colour stops assigned to a gradient.
 * 
 * @author Ed
 */
public class LinearGradientFillStyle extends GradientFillStyle {
	...
	private Map<Double, Colour> colourStops;
	 ...
	 * 
	 * @param where
	 *            A number between 0..1 that defines a value between start and
	 *            end position.
	 * @param colour
	 *            The colour that the gradient should have at this position.
	 */
	 ...
	 * 
	 * @param position
	 *            The position to retrieve the value for
	...
	/*
	 * public String getColourStopsAsJSString() { final StringBuffer sb = new
	 * StringBuffer("["); for (Map.Entry<Double, Colour> stop:
	 * colourStops.entrySet()) {
