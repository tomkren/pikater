...
/**
 * A RGBA colour value. Used for defining all sorts of colours using red, green,
 * blue and alpha (opacity) components.
 * 
 * @author Ed
 */
public class Colour implements FillStyle, Serializable {
	 ...
	 * 
	 * @param alpha
	 *            Defines the transparency (0.0 is completely transparent and
	 *            1.0 is opaque)
	 * @return a translucent colour
	 */
	public Colour alpha(float alpha) {
		return new Colour(this.redComponent, this.greenComponent,
				this.blueComponent, alpha);
	}

	/**
	 * Parametrized Ctor. Alpha is set to 255;
	 * 
	 * @param red
	 *            Red component (0..255)
	 * @param green
	...
	public Colour(int red, int green, int blue, float alpha) {
		...
		if (blue > 255)
			...
			this.blueComponent = 0;
		if (alpha > 1.0f)
			this.alphaComponent = 1.0f;
		else if (alpha < 0.0f)
			this.alphaComponent = 0.0f;
	}
	 ...
	 * 
	 * @param webColour
	 *            A colour string (e.g., '#FF0000') or an rgba string, for
	...
	public Colour(final String webColour) {
		...
		if (webColour.startsWith("#")) {
			...
			greenComponent = Integer.parseInt(webColour.substring(oldPos + 1,
					pos));
			if (greenComponent > 255)
				greenComponent = 255;
			else if (greenComponent < 0)
			...
			pos = webColour.indexOf(",", pos + 1);
			blueComponent = Integer.parseInt(webColour.substring(oldPos + 1,
					pos));
			if (blueComponent > 255)
				blueComponent = 255;
			else if (blueComponent < 0)
			...
			pos = webColour.indexOf(")", pos + 1);
			alphaComponent = Float.parseFloat(webColour.substring(oldPos + 1,
					pos));
			if (alphaComponent > 1.0f)
				alphaComponent = 1.0f;
			else if (alphaComponent < 0.0f)
				alphaComponent = 0.0f;
			oldPos = pos;
	...
	/**
	 * Create a web colour code (#RRGGBB) from the three separate RGB
	 * components.
	 * 
	...
	public String toRGBString() {
		...
		sb.append(redComponent < 16 ? "0" : "").append(
				Integer.toHexString(redComponent));
		sb.append(greenComponent < 16 ? "0" : "").append(
				Integer.toHexString(greenComponent));
		sb.append(blueComponent < 16 ? "0" : "").append(
				Integer.toHexString(blueComponent));
		return sb.toString();
	}
	...
	public String toString() {
		return new StringBuilder("rgba(").append(redComponent).append(",")
				.append(greenComponent).append(",").append(blueComponent)
				.append(",").append(alphaComponent).append(")").toString();
	}
}
