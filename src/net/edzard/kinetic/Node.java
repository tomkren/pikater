...
/**
 * Base class for all of the Kinetic classes. Contains functionality for:
 * <ul>
 * <li>Event handling
 * <li>Naming
...
public abstract class Node extends JavaScriptObject {
	...
	/**
	 * Protected Ctor. Makes JSNI (GWT) happy
	 */
	protected Node() {
	}
	...
	/**
	 * If used with groups and shapes, the layer is cleared, and just that node
	 * is drawn.
	 */
	...
	/**
	 * Gets attribute value for specified key. Can be Kinetic attrs and custom
	 * attrs as well.
	 * 
	 * @param attr
	 *            the key
	...
	/**
	 * Sets an attribute value for specified key. Can be used to set Kinetic
	 * attrs, or even custom attrs. TODO: Custom attr changes can also be
	 * subscribed to, just like Kinetic attrs. i.e. shape.on('myAttrChange',
	 * ...);
	 * 
	 * @param attr
	 *            the key - boxed value
	...
	/**
	 * Gets the type of this node. One of: "Stage", "Layer", "Group", or
	 * "Shape".
	 * 
	 * @return node type
	 */
	...
	/**
	 * Gets the class name of this node, e.g. "Stage", "Layer", "Circle", "Rect"
	 * etc.
	 * 
	 * @return class name
	 */
	...
	/**
	 * Sets the width of the node. Uses bounding boxes around select nodes. For
	 * example, to define the size of an ellipse, you could set its width and
	 * height rather than setting the radius.
	 * 
	 * @param width
	 *            pixels in width
	 */
	...
	/**
	 * Sets the height of the node. Uses bounding boxes around select nodes. For
	 * example, to define the size of an ellipse, you could set its width and
	 * height rather than setting the radius.
	 * 
	 * @param width
	 ...
	 * 
	 * @param size
	 *            The node's new size (x component is width, y component is
	 *            height)
	 */
	 ...
	 * 
	 * @return A value between 0 and 1. 0 is complete transparency and 1 is
	 *         complete opaque.
	 */
	...
	/**
	 * Makes this node visible. Used in conjunction with {@link #hide()}
	 */
	...
	/**
	 * Hides this node. Used in conjunction with {@link #show()}
	 */
	 ...
	 * 
	 * @param newContainer
	 *            The container to move this node to.
	 */
	...
	/**
	 * Moves the node to the highest Z position. It will be in front of all of
	 * the other nodes on a given layer. Node needs to belong to a layer.
	 */
	...
	/**
	 * Moves the node to the lowest Z position. It will be behind all of the
	 * other nodes on a given layer. Node needs to belong to a layer.
	 */
	...
	/**
	 * Increases the Z position of the node. Node needs to belong to a layer.
	 */
	...
	/**
	 * Decreases the Z position of the node. Node needs to belong to a layer.
	 */
	...
	/**
	 * Get the current Z position of the node relative to the container. Node
	 * needs to belong to a layer.
	 * 
	...
	/**
	 * Sets the Z position of the node. Node needs to belong to a layer.
	 * 
	 * @param z
	 *            A new Z position
	 */
	 ...
	 * 
	 * @param listening
	 *            True, if events should be handled. False, otherwise.
	 */
	...
	/**
	 * Set a name for this node. Names do not have to be unique - two nodes with
	 ...
	 * 
	 * @param opacity
	 *            A value between 0 and 1. 0 is complete transparency and 1 is
	 *            complete opaque.
	 */
	 ...
	 * 
	 * @return The two scaling components. The x component holds the horizontal
	 *         scale, while y holds the vertical scale.
	 */
	 ...
	 * 
	 * @param scale
	 *            The two scaling components. The x component holds the
	 *            horizontal scale, while y holds the vertical scale.
	 */
	...
	/**
	 * Retrieve the node's center offset. The center offset is used during
	 * transformations (positioning, scaling, rotation)
	 * 
	...
	/**
	 * Assign the node's center offset. The center offset is used during
	 * transformations (positioning, scaling, rotation)
	 * 
	 * @param offset
	 *            The node center's offset
	 */
	 ...
	 * 
	 * @param drag
	 *            True, if the node should be dragable. False, if not.
	 */
	 ...
	 * 
	 * @param type
	 *            all the events to register the handler for
	 * @param handler
	 *            the handler
	 */
	public final void addEventListener(IEventListener handler,
			NamedEventType... events) {
		addEventListeners(eventListToString(events), handler);
	}

	/**
	 * Convert an array of event types to a single string, as defined by the
	 * KineticJS standard.
	 * 
	 * @param eventTypes
	 *            The event types
	 ...
	 * 
	 * @param eventString
	 *            The string containing events to register.
	 * @param handler
	 *            The handler to execute.
	 */
	// Seems to be buggy in kineticjs. Always getting mousemove events
	// TODO Maik: It's not buggy, it just fails and then creates too many events
	// due to the failure.
	// the evt variable is directly from the browser, not from kineticjs, that's
	// why there's
	...
	// TODO: SkyCrawl: I may be wrong (haven't tested it) but I think there's
	// not going to be a problem with this anymore. Event system has changed
	// quite a bit.
	private final native void addEventListeners(String eventString,
	...
	/**
	 * Remove an event listener from the node for designated events (either
	 * basic or named).
	 * 
	 * @param eventTypes
	 *            A number of event types to stop listening to
	 */
	 ...
	 * 
	 * @param eventTypes
	 *            A (possibly) number of event types to stop listening to
	 */
	...
	/**
	 * Simulates the firing of an event. This will trigger the appropriate
	 * listeners with empty event objects.
	 * 
	 * @param type
	...
	/**
	 * Write the current node's definition to JSON. Doesn't serialize functions
	 * and images. These need to be written manually.
	 * 
	...
	/**
	 * Writes the current node's definition to JSON. Only serializes attributes
	 * defined in the parameter. Doesn't serialize functions, images, DOM
	 * objects or objects with methods.
	 * 
	 * RED ALERT: this method uses a modified version of kinetic's "toJSON"
	 * method (see below) which is not a part of the standard KineticJS
	 * distribution and hence this method is deprecated. It is safe to use this
	 * method with the bundled "dev" version, but beware using it in production
	 * with a "min" version for example. Should you choose to do so, you can
	 * always add the modified function yourselves:
	 * 
	 * toMyJSON: function(attrsToPrint) { return JSON.stringify(this.toObject(),
	 * attrsToPrint, '\t'); }
	 * 
	 * @param attrsToPrint
	 *            attributes to serialize Recommended default code:
	 *            JsArrayString jsonAttrsToSerialize = (JsArrayString)
