...
public class ResultDAO extends AbstractDAO<JPAResult> {
	...
	public List<JPAResult> getResultsByDataSetHashAscendingUponErrorRate(String dataSetHash, int count) {
		return getByTypedNamedQuery("Result.getByDataSetHashErrorAscending", "hash", dataSetHash, 0, count);
	}
	...
	public JPAResult getByExperimentBestResult(JPAExperiment experiment) {
		...
		try {
			return em.createNamedQuery("Result.getByExperimentErrorAscending", JPAResult.class).setParameter("experiment", experiment).setMaxResults(1)//fix exception, when more than one result is returned
					.getSingleResult();
		} catch (Exception e) {
			return null;
		} finally {
