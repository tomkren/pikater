...
{
	private static final String sessionKey_userID = "sk_uid";
	...
	public static Integer getUserID(VaadinSession session)
	{
		...
		{
			return (Integer) session.getAttribute(sessionKey_userID);
		}
	}
	...
	private static void login(VaadinSession session, int userID)
	{
		...
		{
			session.setAttribute(sessionKey_userID, userID);
	...
	public static void logout(VaadinSession session)
	{
		if(isUserAuthenticated(session))
		{
			...
			session.setAttribute(sessionKey_userID, null);
