	...
	public DBTableItemPropertyGeneric(DBTableContainer container, AbstractTableRowDBView row, ITableColumn column, AbstractDBViewValue<T> valueWrapper) {
		...
		if (getValue() != null) {
			...
			if (!valueTypeClass.equals(viewBindingTypeClass)) {
				throw new ClassCastException(String.format("Incorrect type binding detected. '%s' suggests '%s' but '%s' returns '%s'", column.getClass().getName(), viewBindingTypeClass.getName(),
						valueWrapper.getClass().getName(), valueTypeClass.getName()));
			}
		}
	}
