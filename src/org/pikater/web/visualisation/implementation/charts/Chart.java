...
public abstract class Chart {
	...
	protected void renderBackground() {
		...
		renderer.drawLine(offsetx + yLabelWidth, offsety, offsetx + yLabelWidth, offsety + getAreaHeight(), verticalAxis.getAxisColor(), 1);
		//and the ticks
		for (int i = 0; i < verticalAxis.getTickCount(); i++) {
			...
			renderer.drawLine(offsetx + yLabelWidth - tickSize, y, offsetx + yLabelWidth, y, verticalAxis.getAxisColor(), 1);

			if (tickLabelVisible) {
				if (verticalAxis instanceof CategoricalAxis) {
					if (i % 2 == 0) {
						renderer.drawText(verticalAxis.getTickString(i), offsetx + yLabelWidth - tickSize - 2, y + 5, TextAlignment.Center, verticalAxis.getAxisColor(), -90);
					} else {
						renderer.drawText(verticalAxis.getTickString(i), offsetx + yLabelWidth - tickSize - 18, y + 5, TextAlignment.Center, verticalAxis.getAxisColor(), -90);
					}
				} else {
					renderer.drawText(verticalAxis.getTickString(i), offsetx + yLabelWidth - tickSize - 2, y + 5, TextAlignment.Right, verticalAxis.getAxisColor());
				}
			}
		}

		//drawing the horizontal axis
		renderer.drawLine(offsetx + yLabelWidth, offsety + getAreaHeight(), offsetx + getChartWidth(), offsety + getAreaHeight(), horizontalAxis.getAxisColor(), 1);
		//and the ticks
		for (int i = 0; i < horizontalAxis.getTickCount(); i++) {
			...
			renderer.drawLine(x, offsety + getAreaHeight(), x, offsety + getAreaHeight() + tickSize, horizontalAxis.getAxisColor(), 1);
			if (tickLabelVisible) {
				//if we have categories write them in two rows
				if (horizontalAxis instanceof CategoricalAxis) {
					if (i % 2 == 0) {
						renderer.drawText(horizontalAxis.getTickString(i), x, offsety + getAreaHeight() + tickSize + 15, TextAlignment.Center, horizontalAxis.getAxisColor());
					} else {
						renderer.drawText(horizontalAxis.getTickString(i), x, offsety + getAreaHeight() + tickSize + 31, TextAlignment.Center, horizontalAxis.getAxisColor());
					}
				} else {
					renderer.drawText(horizontalAxis.getTickString(i), x, offsety + getAreaHeight() + tickSize + 15, TextAlignment.Center, horizontalAxis.getAxisColor());
				}
