...
public class ValueType implements Concept, IValidated, ICloneable {
	...
	/**
	 * Required to be set (also by {@link Value} class). Should not be
	 * arbitrarily changed.
	 */
	...
	/**
	 * @param defaultValue
	 *            Seriously, don't try to pass in a null...
	 */
	 ...
	 */
	private ValueType(IValueData defaultValue,
			RangeRestriction rangeRestriction, SetRestriction setRestriction) {
	...
	/**
	 * Conditions checked in this method are assumed in web package, so don't
	 * change them lightly:
	 */
	@Override
	public boolean isValid() {
		if (defaultValue == null) {
			...
			if (!rangeRestriction.isValid()) {
				...
				return rangeRestriction
						.validatesValue((IComparableValueData) defaultValue);
			} else if (defaultValue instanceof QuestionMarkRange) {
				QuestionMarkRange qmr = (QuestionMarkRange) defaultValue;
				if (!qmr.isValid()) {
					return false;
				} else {
					return rangeRestriction.isMasterRangeOf(qmr
							.getUserDefinedRestriction());
				}
			} else {
				return false;
			}
			...
			if (!setRestriction.isValid()) {
				return false;
					...
					|| (defaultValue instanceof NullValue)
					|| (defaultValue instanceof QuestionMarkRange)) {
				return false;
				...
				if (!qms.isValid()) {
					...
					return setRestriction.isMasterSetOf(qms
							.getUserDefinedRestriction());
				}
			} else {
	...
	public ValueType clone() {
		...
		RangeRestriction rangeRestrictionClone = null;
		if (rangeRestriction != null) {
			rangeRestrictionClone = rangeRestriction.clone();
		}
		SetRestriction setRestrictionClone = null;
		if (setRestriction != null) {
			setRestrictionClone = setRestriction.clone();
		}
		result.setDefaultValue(defaultValue.clone());
		result.setRangeRestriction(rangeRestrictionClone);
		result.setSetRestriction(setRestrictionClone);
		return result;
	}
}
