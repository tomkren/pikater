...
public class ComputationDescription implements Concept {
	...
	public UniversalComputationDescription exportUniversalComputationDescription() {
		...
		uModel.getGlobalOptions().addAll(
				CollectionUtils.deepCopy(getGlobalOptions()));

		// map - id x ontology
		Map<Integer, UniversalElementOntology> finishedtUniOntologys = new HashMap<Integer, UniversalElementOntology>();
		Map<Integer, IComputationElement> finishedDataProcessings = new HashMap<Integer, IComputationElement>();
		...
		while (!fifo.isEmpty()) {
			...
			if (!finishedDataProcessings.containsKey(dataProcessing.getId())) {
				...
				UniversalElementOntology uOntology = dataProcessing
						.exportUniversalOntology();

				finishedtUniOntologys.put(uOntology.getId(), uOntology);
				finishedDataProcessings.put(dataProcessing.getId(),
						dataProcessing);

				for (DataSourceDescription descrI : dataProcessing
						.exportAllDataSourceDescriptions()) {
		...
		for (Integer keyI : finishedtUniOntologys.keySet()) {
			...
			for (ISourceDescription slotIJ : processI
					.exportAllDataSourceDescriptions()) {

				UniversalElement uniElement = new UniversalElement();
				UniversalElementOntology uniOntology = finishedtUniOntologys
						.get(slotIJ.exportSource().getId());
	...
	public static ComputationDescription importUniversalComputationDescription(
		...
		description.setGlobalOptions(new ArrayList<NewOption>(uDescription
				.getGlobalOptions()));
		...
		while (!fifo.isEmpty()) {
			...
			if (!finishedDataProcessings.containsKey(uOntology.getId())) {
				...
				DataProcessing ontology = DataProcessing
						.importUniversalOntology(uOntology);

				finishedtUniOntologys.put(uOntology.getId(), uOntology);
				finishedDataProcessings.put(uOntology.getId(), ontology);

				for (UniversalElementConnector connectorI : uElement
						.getOntologyInfo().getInputDataSlots()) {
		...
		for (Integer keyI : finishedDataProcessings.keySet()) {
			...
			DataProcessing processI = (DataProcessing) finishedDataProcessings
					.get(keyI);
			...
			for (UniversalElementConnector slotIJ : uOntoI.getInputDataSlots()) {
				...
				IDataProvider dataProvider = (IDataProvider) finishedDataProcessings
						.get(uElementID);
			...
			for (UniversalElementConnector slotIJ : uOntoI.getInputErrorSlots()) {
				...
				UniversalElement uElement = slotIJ.getFromElement();
				IDataProvider dataProvider = (IDataProvider) finishedDataProcessings
						.get(uElement.getOntologyInfo().getId());
