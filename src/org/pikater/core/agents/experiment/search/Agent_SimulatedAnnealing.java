 ...
 * <li>-S float Stability of generation of new option - probability of keeping
 * an option (default 0.5)
 * </ul>
 */
public class Agent_SimulatedAnnealing extends Agent_Search {
	...
	/**
	 * Loads the parameters of annealing from search's options. If the options
	 * are not set, sets the values to defaults:
	 * 
	 * <ul>
	 * <li>temperature = 1.0;
	 * <li>maximumTries = 50;
	...
	protected void loadSearchOptions() {
		...
		FloatValue valueS = (FloatValue) optionS.toSingleValue()
				.getCurrentValue();
		stability = valueS.getValue();

		NewOption optionT = options.fetchOptionByName("T");
		FloatValue valueT = (FloatValue) optionT.toSingleValue()
				.getCurrentValue();
	...
	/**
	 * Generates a new solution (uses the {@link neighbor} method). If it is the
	 * first solution to generate, initialize the search first.
	 */
	@Override
	...
	protected float updateFinished(float[][] evaluations) {
		...
		/*
		 * System.out.println(", e1:" + newEvaluation + ", acceptance: " + acc +
		 * " ,.5->1: " + acceptanceProb(1-0.5,temperature) + " ,1->.5:"+
		 * acceptanceProb(0.5-1,temperature));
		 */
	...
	/**
	 * Generates random solutions in the beginning, or changes the existing
	 * solution.
	 * 
	 */
