...
public class Types {
    ...
    /*
    public static Type parse_old(String str) {

        String str2 = str.replaceAll("\\("," \\( ").replaceAll("\\)"," \\) ").trim();
        ...
        for (String part : parts) {
            ...
            if (varId != -1) { // var
                stack.add( new TypeVar(varId) );

            } else {
                stack.add( new TypeSym(part) );
            }
    ...
    private static int idFromVarStr(String str) {
        if (str != null && str.length() > 1 && str.charAt(0) == 'x') {
            try {
